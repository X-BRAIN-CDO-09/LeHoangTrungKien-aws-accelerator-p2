...
LOG_DIR="${LOG_DIR:-/tmp/w10-port-forward}"
...
start_forward() {
  ...
  echo "${label}: http://localhost:${local_port}  ->  ${namespace}/svc/${service}:${remote_port}"
  echo "  log: ${log_file}"
}
...
usage() {
  cat <<'USAGE'
Usage:
  ./cloud/w10/security-hardening-lab/scripts/port-forward-w10.sh [all|app|obs|argocd]
...
if [[ "$MODE" == "all" || "$MODE" == "argocd" ]]; then
  start_forward argocd argocd-server 8080 443 "ArgoCD"
fi
...
if [[ "$MODE" == "all" || "$MODE" == "app" ]]; then
  start_forward flipkart flipkart-frontend 8081 80 "Flipkart frontend"
  start_forward flipkart flipkart-backend 4000 4000 "Flipkart backend stable"
  start_forward flipkart flipkart-backend-canary 4001 4000 "Flipkart backend canary"
fi
